...
def get_same_padding(kernel_size: int or Tuple[int, ...]) -> int or Tuple[int, ...]: #填充大小padding
...
def build_kwargs_from_config(config: dict, target_func: callable) -> Dict[str, any]:
    valid_keys = list(signature(target_func).parameters) #Obtain the parameter list
    kwargs = {}
    for key in config:
...
def build_norm(name="bn2d", num_features=None, **kwargs) -> nn.Module or None:
    ...
    if name in REGISTERED_NORM_DICT:
        ...
        args = build_kwargs_from_config(kwargs, norm_cls) #Build a parameter dictionary
...
class ConvLayer(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size=3, stride=1, dilation=1, groups=1,
        ...
        padding = get_same_padding(kernel_size) #Get the recharge value
        padding *= dilation
        ...
        self.act = build_act(act_func) #Get the recharge value
...
class LiteMLA(nn.Module):
        ...
        self.aggreg = nn.ModuleList(
            [nn.Sequential(
                nn.Conv2d(3 * total_dim, 3 * total_dim, scale, padding=get_same_padding(scale), groups=3 * total_dim,
                          bias=use_bias[0], ), #groups The number of group convolution Each channel group has its own convolution kernel. Depthwise separable convolution = depthwise convolution + fully connected layer
    ...
    def relu_linear_att(self, qkv: torch.Tensor) -> torch.Tensor:
        ...
        q, k, v = (qkv[..., 0: self.dim], qkv[..., self.dim: 2 * self.dim], qkv[..., 2 * self.dim:],) #取最后维度的切片
        ...
        # linear matmul
        trans_k = k.transpose(-1, -2) #transposition


        v = F.pad(v, (0, 1), mode="constant", value=1) #Finally, add one dimension and fill it with 1
